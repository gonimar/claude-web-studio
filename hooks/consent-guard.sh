#!/bin/bash
# PreToolUse(Write|Edit|Bash): warn-only consent sentinel for pipeline documents.
...
# Bash counts as a write: the skills author documents with `cat > … <<EOF` and `python3 - <<PY`, so a
# Write|Edit-only sentinel watched a lane nobody used and did not fire once in a whole day (WS-085).
