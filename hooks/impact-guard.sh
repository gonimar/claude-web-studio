#!/bin/bash
# PreToolUse(Write|Edit|Bash): warn-only impact sentinel for architecture/security surfaces.
...
# Code written through a Bash heredoc reaches the same surfaces, and that is how the skills
# write in practice — so the hook reads Bash commands too (WS-085).
...
sensitive() {
  case "$1" in
    */docs/*|docs/*|*/production/*|production/*|*/.claude/*|.claude/*|*/node_modules/*|*/vendor/*) return 1;;
  esac
  case "$1" in
...
[ -z "$PATHS" ] && exit 0
HIT=""
for p in $PATHS; do sensitive "$p" && HIT="$HIT $p"; done
[ -z "$HIT" ] && exit 0
FP=$(echo $HIT)
