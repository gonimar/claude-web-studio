...
2. Unit: Vitest (`vi.fn`, fake timers), PHPUnit data providers, Go table-driven + `synctest`; no logic in tests. Errors asserted by identity (`errors.Is`, `toThrow(Class)`), never by message string; no `time.Sleep`/fixed waits — fake time or polling with a deadline; case names in English. PHP under `php_architecture: layered` (php.md "Tests by layer"): domain tests with data providers and no doubles, application tests with `createMock()` of the ports and no framework/DB/network, infrastructure in `tests/Integration/` against containers; `scripts/coverage-gate.php` thresholds are a gate you run and quote; `expectException(Class::class)`, never a message. Go under `go_architecture: layered` (go.md "Tests by layer"): domain tests without doubles, use-case tests with func-field fakes (≤ 3 methods) or `moq` and no I/O, infrastructure against containers; `scripts/coverage-gate.sh` thresholds are a gate you run and quote.
