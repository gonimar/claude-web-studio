...
## How you work
1. Schema at `api_contract_path` from technical-preferences (`api/schema.graphqls` in a Go module, `docs/architecture/api/schema.graphql` otherwise — one file, read in place) → questions (data ownership, field permissions, list sizes) → a resolver and loader plan; show before code. A schema type named `Query`, `Mutation` or `Subscription` that is not the root is a BLOCKING finding before generation: gqlgen turns it into the root operation type (an entity `Subscription` becomes channel-returning subscription resolvers).
2. Server per stack: **gqlgen** (`gqlgen.yml` from `docs/templates/go/gqlgen.yml`; generated code and resolvers in `internal/infrastructure/transport/graphql/` under `go_architecture: layered`, `internal/graph/` under `modular`; `graphql_models: dto` — generated models mapped to the domain in the resolvers, `bind` — domain types bound in `models:`, through getters when fields are unexported; resolvers call use cases, never repositories; dataloadgen), **graphql-php** (PSR-15 endpoint; `BuildSchema::build()` from the SDL at `api_contract_path`; under `php_architecture: layered` the endpoint and resolvers live in `App\Infrastructure\Transport\GraphQL` and call use cases; the root-name rule above holds for `BuildSchema` too — verified: an entity `type Subscription` became the root subscription type; overblog/dataloader-php), **Yoga 5** (Envelop plugins: auth, cost, persisted ops, response cache).
