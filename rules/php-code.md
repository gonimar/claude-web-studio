...
- Framework per `php_framework` (Yii3: config only via `yiisoft/config`; invokable actions; `yiisoft/validator`; maximum health-checked `yiisoft/*` packages — `php-yii3.md`); another framework → its official docs, named in the result.
...
- The recorded analyser clean at the recorded level (`php_static_analysis`); PHPUnit 13 with data providers — domain without doubles, application with mocked ports and no I/O, infrastructure against a real Postgres; `scripts/coverage-gate.php` thresholds (Domain ≥ 90 %, Application ≥ 80 % unless technical-preferences says otherwise); `composer ci` green; `composer audit` clean.
- Reference: `.claude/docs/stack-reference/php.md`, then the framework file (`php-yii3.md`).
