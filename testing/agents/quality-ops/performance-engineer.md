...
## Summary
**Domain**: Measurements and improvements: CWV/Lighthouse, bundle, k6, EXPLAIN, pprof, game frame
**Does not own**: Functional changes
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/web-platform.md, database.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `performance-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/web-platform.md, database.md` first
...
### 2. Out of domain — redirect to backend-lead / frontend-lead
...
### 3. Standard violated — optimising without measuring
**Scenario**: the spec/existing code requires "optimising without measuring". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — budgets from technical-preferences
**Scenario**: the context "budgets from technical-preferences" is passed. **Expected**: uses it without re-asking, does not expand the task.
