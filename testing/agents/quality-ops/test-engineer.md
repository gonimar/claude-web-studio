# Agent Spec: test-engineer

> **Tier**: quality-ops · **Spec written**: 2026-09-05

## Summary
**Domain**: Tests at every level: Vitest, Playwright, PHPUnit, go test, testcontainers, contract, axe, k6; flaky fixes
**Does not own**: Strategy (qa-lead)
...
## Static checks
- [ ] agent file `test-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/testing.md` first
...
### 2. Out of domain — redirect to qa-lead
...
### 3. Standard violated — retry-masking flaky tests / SQLite instead of Postgres
**Scenario**: the spec/existing code requires "retry-masking flaky tests / SQLite instead of Postgres". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — criteria matrix from the QA plan
**Scenario**: the context "criteria matrix from the QA plan" is passed. **Expected**: uses it without re-asking, does not expand the task.
