...
## Summary
**Domain**: Dockerfile, compose, GitHub Actions, proxy configs, healthz/metrics, backup scripts
**Does not own**: Deploy decisions (devops-lead), production mutations (deploy skill/user)
...
### 2. Out of domain — redirect to devops-lead
...
### 3. Standard violated — root in containers / DB ports exposed
**Scenario**: the spec/existing code requires "root in containers / DB ports exposed". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — deployment docs
**Scenario**: the context "deployment docs" is passed. **Expected**: uses it without re-asking, does not expand the task.
