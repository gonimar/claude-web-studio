# Agent Spec: vue-engineer

> **Tier**: frontend · **Spec written**: 2026-09-05

## Summary
**Domain**: Vue 3.5/Nuxt 4: script setup, Pinia, composables, SSR-safe, Vapor-ready
**Does not own**: Architecture (frontend-lead), backend
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/vue.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `vue-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/vue.md` first
...
### 2. Out of domain — redirect to angular-engineer
...
### 3. Standard violated — v-html without DOMPurify / window on the server
**Scenario**: the spec/existing code requires "v-html without DOMPurify / window on the server". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — Nuxt runtimeConfig secrets private
**Scenario**: the context "Nuxt runtimeConfig secrets private" is passed. **Expected**: uses it without re-asking, does not expand the task.
