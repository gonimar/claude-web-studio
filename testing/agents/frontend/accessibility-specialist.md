...
## Summary
**Domain**: WCAG 2.2 AA audit and fixes, axe, keyboard, ARIA, game settings
**Does not own**: Visual design (design-lead)
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/web-platform.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `accessibility-specialist.md` with `name/description/model/tools`
- [ ] reads `stack-reference/web-platform.md` first
...
### 2. Out of domain — redirect to css-engineer / design-lead
...
### 3. Standard violated — ARIA instead of a native element
**Scenario**: the spec/existing code requires "ARIA instead of a native element". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — routes from the UX spec
**Scenario**: the context "routes from the UX spec" is passed. **Expected**: uses it without re-asking, does not expand the task.
