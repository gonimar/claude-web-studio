# Agent Spec: angular-engineer

> **Tier**: frontend · **Spec written**: 2026-09-05

## Summary
**Domain**: Angular 22: standalone, signals, Signal Forms, zoneless, Material 22/Taiga 5, SSR, Vitest
**Does not own**: Client architecture (frontend-lead), backend
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/angular.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `angular-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/angular.md` first
...
### 2. Out of domain — redirect to vue-engineer / go-engineer
...
### 3. Standard violated — NgModule / *ngIf / subscription without takeUntilDestroyed
**Scenario**: the spec/existing code requires "NgModule / *ngIf / subscription without takeUntilDestroyed". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — design system on Taiga
**Scenario**: the context "design system on Taiga" is passed. **Expected**: uses it without re-asking, does not expand the task.
