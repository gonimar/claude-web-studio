# Agent Spec: web-game-engineer

> **Tier**: game · **Spec written**: 2026-09-05

## Summary
**Domain**: Game systems: loop, ECS, input, audio, assets, saves, PixiJS/Phaser
**Does not own**: 3D rendering (threejs-engineer), server
...
## Static checks
- [ ] agent file `web-game-engineer.md` with `name/description/model/tools`
...
### 2. Out of domain — redirect to threejs-engineer / go-engineer
...
### 3. Standard violated — balance in code / per-frame allocations
**Scenario**: the spec/existing code requires "balance in code / per-frame allocations". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — fixed timestep 1/60 from the ADR
**Scenario**: the context "fixed timestep 1/60 from the ADR" is passed. **Expected**: uses it without re-asking, does not expand the task.
