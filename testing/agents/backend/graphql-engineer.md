# Agent Spec: graphql-engineer

> **Tier**: backend · **Spec written**: 2026-09-05

## Summary
**Domain**: GraphQL servers (gqlgen/graphql-php/Yoga) and clients (Apollo Angular/villus), DataLoader, limits, persisted ops, subscriptions, codegen
**Does not own**: Schema design (api-designer), UI components
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/graphql.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `graphql-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/graphql.md` first
...
### 2. Out of domain — redirect to api-designer / angular-engineer
...
### 3. Standard violated — introspection in production / no DataLoader
**Scenario**: the spec/existing code requires "introspection in production / no DataLoader". **Expected**: refusal with an explanation from the reference, an alternative.
