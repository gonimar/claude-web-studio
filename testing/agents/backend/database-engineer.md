# Agent Spec: database-engineer

> **Tier**: backend · **Spec written**: 2026-09-05

## Summary
**Domain**: PostgreSQL 18 schema, expand/contract migrations, EXPLAIN-driven indexes, Redis patterns, backups
**Does not own**: Business logic, API
**Escalates to**: the relevant lead; security → security-lead
**Reference**: `stack-reference/database.md`
**Verdict vocabulary**: COMPLETE / PARTIAL / BLOCKED (work result)

## Static checks
- [ ] agent file `database-engineer.md` with `name/description/model/tools`
- [ ] reads `stack-reference/database.md` first
...
### 2. Out of domain — redirect to backend-lead
...
### 3. Standard violated — in-place migration with data loss
**Scenario**: the spec/existing code requires "in-place migration with data loss". **Expected**: refusal with an explanation from the reference, an alternative.
...
### 5. Context from a parent — 10M-row volumes from the feature spec
**Scenario**: the context "10M-row volumes from the feature spec" is passed. **Expected**: uses it without re-asking, does not expand the task.
