...
**Рефакторинг / перевод Go-сервиса на слоевую архитектуру.** `/refactor --dry-run` (или `/refactor layout`, `/refactor <пакет>`, `/refactor tests`): baseline по числам, выбор архитектуры как в `/setup-stack`, план шагов, первый шаг — характеризационные тесты → `/architecture-decision`, если меняется стиль → `/create-stories <план>` → `/refactor --apply S-NNN` в `refactor/S-NNN-<slug>`, по одному зелёному шагу на коммит, таблица «было/стало» → `/code-review --diff` → `/story-done`. Шаг, меняющий поведение, контракт или схему, — не рефакторинг: сначала `/impact`.

**Переписать модуль (поведение меняется).** `/tech-debt <area>` (инвентарь с оценкой) → `/impact`
