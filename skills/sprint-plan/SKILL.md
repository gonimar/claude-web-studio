...
**Calibration**: from the roadmap, every closed story that carries both `~Nh` and `⏱ Nh` → ratio = Σ⏱ / Σ~ (the last `/retrospective` reports the same number); with three or more such stories the ratio is applied to this sprint's estimates (calibrated estimate = `~Nh` × ratio) and shown as one line `Calibration: ratio R over N stories — capacity fits ~X h of estimates`; fewer than three → "insufficient data, estimates taken as written". The ratio is never rounded towards a nicer plan.
