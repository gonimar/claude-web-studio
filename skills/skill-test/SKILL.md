...
4. Ask-before-write: `May I write` (or an explicit gate sentence) when `Write|Edit` is in `allowed-tools` — FAIL; otherwise WARN.
5. A "Next step" at the end — WARN.
...
Read SKILL.md and the spec `skills/<category>/<name>.md`; for every case and assertion find the instructions in the skill text that satisfy it: PASS/FAIL/PARTIAL with a quoted line. Totals per case and protocol. "May I write the result to `results/<name>-<date>.md` and update `catalog.yaml`?"
...
Verdict: `COMPLIANT` | `WARNINGS` | `NON-COMPLIANT`. Next step: `/skill-improve <name>` for failures.
