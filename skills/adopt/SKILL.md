...
- Go projects: compare the tree with `go.md` "Project layout" (golang-standards/project-layout adapted) — `src/`, `utils/`/`common/`, logic in `cmd/`, an unused `pkg/` → INFO/MEDIUM findings with a migration note; record the actual variant as `go_layout` in technical-preferences (never restructure during adoption). Record the architecture style from the tree, never from a wish: `internal/domain/` + `internal/usecase/` present (or Evrone's `internal/entity/` + `internal/usecase/` + `internal/repo/`) → `go_architecture: layered` (plus `go_layers`, `go_composition_root` from where the graph is built); otherwise `modular`. `go_router` from `go.mod` (chi / none → ServeMux); `graphql_models` from `gqlgen.yml` (`models:` bound to `internal/domain` → bind, else dto); `api_contract_path` from where the SDL actually is. A `.golangci.yml` in v1 format (`linters-settings:`, no `version: "2"`) is a MEDIUM finding — golangci-lint v2 rejects it. Moving to `layered` is offered as `/refactor layout` in the adoption plan, not done here.
- PHP projects: `php_version` from `composer.json` `require.php` and `composer.lock` `platform`/`platform-overrides` (below 8.4 → HIGH finding: the rich-model form and PHPUnit 13 need 8.4, the studio target is 8.5; 8.4 → INFO with an upgrade story to 8.5); `php_framework` from `composer.json` (`yiisoft/*` → yii3; `symfony/framework-bundle` → symfony; `laravel/framework` → laravel; `slim/slim` → slim; none of them → none); `php_architecture` from the tree (`src/Domain` + `src/Application` present → layered, else framework; `php_layers` per-context when `src/Application/*/` holds sub-namespaces, else flat); `php_static_analysis` from `phpstan.neon*`/`psalm.xml`; `php_cs_tool` from `ecs.php`/`.php-cs-fixer*.php`; deptrac/phparkitect/phpat config present or not; PHPUnit major from the lockfile (11 or older → MEDIUM finding, out of bugfix support since 2026-02); DDL in PHP classes outside a migrations directory → MEDIUM finding (`rules/database.md`). A framework without a studio reference is recorded with the line "php-engineer works from the official documentation". Moving to `layered` or to another framework is offered as `/refactor layout` / `/refactor framework --dry-run` in the adoption plan, never done here.
