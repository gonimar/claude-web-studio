...
`test-engineer` via Task: configs, a first smoke test per level, CI stages (`devops-engineer`). Go module: `.golangci.yml` from `docs/templates/go/golangci.yml` (`MODULE_PATH` replaced from `go.mod`; the `depguard` block kept under `layered`, removed under `modular`; `go_domain_allow` entries added to the `domain` allow-list), `scripts/coverage-gate.sh` from `docs/templates/go/coverage-gate.sh` with the thresholds, the `fmt`/`lint`/`test`/`coverage-gate`/`layout-check`/`arch-check`/`ci` targets from `docs/templates/go/Makefile.snippet` merged into the Makefile, `moq` as a `tool` directive when a port has more than three methods; `make ci` runs once and its output is in the result. PHP project: `deptrac.yaml` from `docs/templates/php/deptrac.yaml` (`FRAMEWORK_NAMESPACES` from `php_framework`; kept under `layered`, removed under `framework`; `php_domain_allow` namespaces added to the `Vendor` layer), the analyser config for `php_static_analysis` (`phpstan.neon` / `psalm.xml`), the coding-standard config for `php_cs_tool` (`ecs.php` / `.php-cs-fixer.dist.php`), `phpunit.xml`, `scripts/coverage-gate.php`, the `scripts` block from `composer-scripts.json` merged into `composer.json` with the lines of the chosen tools, `deptrac/deptrac`, the analyser, the standard tool and `phpunit/phpunit:^13` in `require-dev`, pcov in the CI image; `composer ci` runs once and its output is in the result — and the run shows `php scripts/coverage-gate.php Domain=100` failing on the current numbers so the gate is known to work — a gate that has never failed on purpose is not known to work, so the run also shows `scripts/coverage-gate.sh domain=100` failing on the current numbers **within the budget rules of `stack-reference/tooling-devops.md` § CI** — one job per toolchain, `paths:` filters, e2e and security on pull requests to the default branch, `concurrency: cancel-in-progress`, `runs-on: ${{ vars.CI_RUNNER || 'ubuntu-latest' }}`. The report states the estimated minutes per run and per month at the project's current merge rate; on a private repository the free tier is 2 000 minutes a month and every job is rounded up to the minute, so a seven-job pipeline of forty-second jobs costs seven minutes per push. A run — output in the result.
