...
- Backend: `go-engineer` / `php-engineer` / `node-engineer`; GraphQL — `graphql-engineer`; DB — `database-engineer`. Go under `go_architecture: layered`: the step table names the layer of every step (domain → use case → infrastructure → composition root → transport, in that order for a vertical slice), a domain or use-case step includes its tests, and the story result quotes the `coverage-gate` lines and the `golangci-lint` count; a story that would restructure existing packages is not a story, it is `/refactor` (rule 11 detour).
