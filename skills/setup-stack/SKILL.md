...
2. Backend: **Go 1.27** (services, realtime, games) | **PHP 8.5** (content systems, existing PHP ecosystem) | **Node 24** (BFF/SSR) | none. Recommendation by type. PHP → `php_framework`, one `AskUserQuestion`: **Yii3** (Recommended — the only framework with a studio reference, `php-yii3.md`) | Symfony | Laravel | Slim | none (PSR-15 pipeline only); any choice but Yii3 is recorded with the line "php-engineer works from the official documentation; no studio reference yet".
