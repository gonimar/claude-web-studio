...
The verifier's contract, at most 15 lines: `APPROVED` · `APPROVED WITH CONDITIONS (…)` · `NEEDS ADR` · `BLOCKED (reason)`; then the artifacts that must change (ADR, threat-model surface, contract, data model, spec, stories) and **the commands to run, in order** — a verdict without commands is incomplete and goes back once with that request; a second incomplete reply is reported as such, never padded by the skill. A `BLOCKED` from any verifier is surfaced immediately with the reason.
